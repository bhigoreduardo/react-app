...
function Sidebar() {
  ...
  const normalLink = 'flex items-center gap-5 rounded-lg text-md capitalize m-2 p-2 dark:text-gray-200 dark:hover:text-slate-900 dark:hover:bg-light-gray hover:bg-secondary-dark-bg hover:text-white';
  const activeLink = `${normalLink} ${theme === 'light' ? 'text-white' : 'text-secondary-dark-bg'}`;
  ...
  return (
    <article className="flex flex-col gap-10 min-h-screen p-5 dark:bg-secondary-dark-bg bg-light-gray overflow-auto">
      ...
      {
        sidebarLinks?.length > 0 && sidebarLinks.map((item) => (
          <div key={item.title}>
            <p className="font-bold text-gray-400 uppercase text-center">{item.title}</p>
            {
