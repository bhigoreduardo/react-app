...
function Navbar() {
  const { isSidebar, setIsSidebar, color, iconNavState, setIconNavState } = useStateContext();
  ...
  return (
    <nav className="flex items-center flex-wrap md:justify-end justify-between px-2 py-1 m-2 dark:bg-secondary-dark-bg bg-light-gray">
      <div className="md:hidden block">
        <NavButton content="Menu" icon={<AiOutlineMenu />} color={color} handleOnClick={() => setIsSidebar(!isSidebar)} />
      </div>

      <div className="flex gap-1">
        <NavButton content="Cart" icon={<FiShoppingCart />} color={color} handleOnClick={() => iconNavbarClicked("cart")} />
        <NavButton content="Chat" icon={<BsChatLeft />} color={color} dotColor="#03C9D7" handleOnClick={() => iconNavbarClicked("chat")} />
        <NavButton content="Notification" icon={<RiNotification3Line />} color={color} dotColor="rgb(254, 201, 15)" handleOnClick={() => iconNavbarClicked("notification")} />
        <TooltipComponent content="Profile" position="BottomCenter" onClick={() => iconNavbarClicked("userProfile")}>
