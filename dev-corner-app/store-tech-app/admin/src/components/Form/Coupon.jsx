...
const FormCoupon = ({ setIsForm }) => {
  ...
  return (
    <form onSubmit={formik.handleSubmit}>
      ...
      <input type="text" id="title" name="title" onChange={formik.handleChange} onBlur={formik.handleBlur} value={formik.values.title} placeholder="Nome do cupom" className="d-block w-100" />
      <div className="error mb-3">{formik.touched.title && formik.errors.title}</div>

      <label htmlFor="expiry">Data expiração</label>
      <input type="date" id="expiry" name="expiry" onChange={formik.handleChange} onBlur={formik.handleBlur} value={formik.values.expiry} placeholder="Data expiração" className="d-block w-100" />
      <div className="error mb-3">{formik.touched.expiry && formik.errors.expiry}</div>
      
      <label htmlFor="discount">Desconto</label>
      <input type="number" id="discount" name="discount" onChange={formik.handleChange} onBlur={formik.handleBlur} value={formik.values.discount} placeholder="Porcentagem desconto" className="d-block w-100" />
      <div className="error mb-3">{formik.touched.discount && formik.errors.discount}</div>
