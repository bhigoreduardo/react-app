...
const BlogForm = ({ setIsForm }) => {
  ...
  return (
    <form onSubmit={formik.handleSubmit}>
      ...
      <ReactQuill theme="snow" name="description" onChange={formik.handleChange("description")} onBlur={formik.handleBlur} value={formik.values.description} className="d-block w-100" />
          ...
          {({ getRootProps, getInputProps }) => (
            <section>
              <div {...getRootProps()}>
                <input {...getInputProps()} />
                <p>
                  Arraste e solte, ou toque para selecionar imagem
                </p>
              </div>
            </section>
          )}
        ...
        {images?.map((item, i) => {
          return (
            <div className="position-relative" key={i}>
              <button type="button" onClick={() => removeImage(i)} className="btn-close position-absolute" style={{ top: "10px", right: "10px" }}></button>
              <img src={window.URL.createObjectURL(item)} alt="" width={200} height={200} />
            </div>
          );
        })}
      </div>
