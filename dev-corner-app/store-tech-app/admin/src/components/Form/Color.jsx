...
const FormColor = ({ setIsForm }) => {
  ...
  return (
    <form onSubmit={formik.handleSubmit}>
      ...
      <input type="text" id="title" name="title" onChange={formik.handleChange} onBlur={formik.handleBlur} value={formik.values.title} placeholder="Nome da cor" className="d-block w-100" />
      <div className="error mb-3">{formik.touched.title && formik.errors.title}</div>
      
      <label htmlFor="hex">Cor</label>
      <input type="color" id="hex" name="hex" onChange={formik.handleChange} onBlur={formik.handleBlur} value={formik.values.hex} placeholder="Nome da cor" className="d-block w-100" />
      <div className="error mb-3">{formik.touched.hex && formik.errors.hex}</div>
