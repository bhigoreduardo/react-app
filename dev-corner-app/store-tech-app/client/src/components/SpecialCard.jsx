...
const SpecialCard = ({ cover, title, brand, stars, offerCreatedAt, offerDeadline, offer, price, stock, stockOffer }) => {
  ...
  return (
    <div className="special-product-card d-sm-flex d-block justify-content-between">
      ...
      <div className="special-product-content col-7">
        ...
        <p className="price"><span className="red-p">{currencyPrice.format(offer)}</span> &nbsp; <strike>{currencyPrice.format(price)}</strike></p>
        <div className="discount-till d-flex align-items-center gap-10">
          <p className="mb-0 me-3">
            <b>{days}&nbsp;</b>dias
          </p>
          <div className="d-flex gap-10 align-items-center">
            <span className="badge rounded-circle p-2 bg-danger">{countdown.getHours()}</span>:
            <span className="badge rounded-circle p-2 bg-danger">{countdown.getMinutes()}</span>:
            <span className="badge rounded-circle p-2 bg-danger">{countdown.getSeconds()}</span>
          </div>
        </div>
        <div className="prod-count my-3">
          <p>Unids restantes: {stock}</p>
          <div className="progress">
            <div className="progress-bar" role="progressbar" style={{ width: `${restStock}%` }} aria-valuenow={stock} aria-valuemin="0" aria-valuemax={stockOffer}></div>
          </div>
        </div>
